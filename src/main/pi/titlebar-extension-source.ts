  ...
  const uiPromptHandlers =
    kind === 'pi'
      ? [
          ...
          '    // Why: only hold the spinner off once the marker is actually up, or a ctx that',
          '    // cannot paint would freeze the title on its last working frame instead.',
          '    try {',
          ...
          '    // The captured ctx is the one the modal may have invalidated, so a throw here',
          '    // must not reject the handler; the running interval or the next turn repaints.',
          '    try {',
          ...
          '    } catch {',
          '      // Why: no live ctx is left to paint through.',
          '    }',
          '  })',
          ''
        ]
